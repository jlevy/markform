...
This is a validation spec for the new `table-field` type implementation in Markform, used to collect structured tabular data with typed columns.
...
- Parsing qualified references: `rating.bullish` → `{ type: 'option', fieldId: 'rating', optionId: 'bullish' }`
- Parsing cell references: `key_people.name[0]` → `{ type: 'cell', fieldId: 'key_people', columnId: 'name', rowIndex: 0 }`
