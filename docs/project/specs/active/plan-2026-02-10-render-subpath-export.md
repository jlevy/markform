...
consumers (e.g. DataRoom Browser) can render forms and fill records with exact visual
parity to `markform serve`, without duplicating ~1800 lines of rendering code.

**GitHub Issue:** #131

**Related docs:**

- `packages/markform/src/cli/commands/serve.ts` — Current home of all rendering
  functions (3,544 lines)
...
All HTML rendering logic lives in a single monolithic file: `serve.ts` (3,544 lines).
This file contains:
...
2. **Full-page HTML renderers** — `renderFormHtml()`, `renderFieldHtml()`, complete page
   shells with CSS/JS
3. **Content-only renderers** — Functions that produce HTML fragments without page
   wrappers:
...
6. **JavaScript for interactivity** — `frShowTip()`, `frHideTip()`, `frCopyYaml()`
   tooltip and copy handlers (defined in inline `<script>`, lines 1014-1039)
7. **Private helpers** — `renderViewFieldValue()`, `formatCheckboxState()`,
   `highlightSourceLine()`, `highlightYamlValue()`, `formatPatchValue()`,
   `renderPatchDetails()`, `renderToolCall()` — used internally by the exported
   functions
...
- No Node.js-specific APIs (fs, http, etc.)
  are used by the rendering functions themselves

### Gap Analysis

External consumers currently cannot:

1. Import rendering functions — they’re only accessible via `markform/cli` which pulls
   in the entire CLI including commander, HTTP server, picocolors, etc.
...
And embed the HTML fragments in their own page shell, providing their own CSS reset,
layout, and surrounding UI.
...
**Cons:**
- `import from 'markform/render'` would pull in the entire serve.ts module at import
  time, including all its imports (commander types, picocolors, node:http, node:fs,
  etc.)
...
`src/render/` module.
The serve command imports from render instead of defining inline.
...
Option A is tempting for simplicity but defeats the purpose — consumers would still pull
in the full CLI module graph.
Option B is more than needed.
Option C is architecturally wrong.
...
The tooltip and copy functions currently live in the serve page shell.
We need to extract them as a string constant that consumers can embed:
...
The render module needs types from the engine.
These are already exported from the main entry point, so consumers can import types from
`markform` and renderers from `markform/render`:
...
The render module itself imports types directly from engine source files (not via the
main entry point), which is the standard pattern in this codebase.
...
- **Code types, methods, and function signatures**: MAINTAIN — all functions keep
