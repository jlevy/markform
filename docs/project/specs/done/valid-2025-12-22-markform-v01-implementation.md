...
- [x] Main help lists all 7 commands (inspect, validate, apply, export, render, serve,
  run)
...
- [x] Checkboxes (multi mode) render as selects with 5 states
...
- [x] All field types recognized: string, number, string_list, single_select,
...
**Note:** Without API key or with insufficient credits, the script falls back to mock
mode. AI SDK tools are verified via unit tests in
`tests/unit/integrations/ai-sdk.test.ts` (26 tests).
...
1. ~~**Apply command `--dry-run`**~~: ✅ RESOLVED - Was already implemented, verified
   working

2. ~~**Export command `markdown` field**~~: ✅ RESOLVED - Added `--markdown` flag to
   export command

3. **Serve auto-open browser**: ✅ RESOLVED - Browser now auto-opens; use `--no-open` to
   disable
...
6. **Tiered priority system**: ✅ ADDED - P1-P5 issue priority based on field importance
   \+ issue type

7. **Global `--format` flag**: ✅ ADDED - Unified format control
   (console/plaintext/yaml/json)
...
- 2025-12-23: Comprehensive validation completed; added validation summary and test
  results

- 2025-12-23: Resolved all discovered issues (markform-82, 83, 84, 88); added render
  command, export --markdown, serve auto-open, verified apply --dry-run

- 2025-12-23: Updated for branch fixups1 - added validate command, tiered priority
  (P1-P5), global --format flag; updated CLI help to show 7 commands
