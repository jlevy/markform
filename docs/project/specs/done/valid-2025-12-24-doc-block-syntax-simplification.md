...
2. **Serializer Tests** (`tests/unit/engine/serialize.test.ts`):
   ...
   - Verifies output uses semantic tag names (`{% description %}`, `{% instructions %}`,
     `{% documentation %}`)
...
2. **Example Forms** (all successfully parsed with new syntax):
   ...
   - `political-research/political-research.mock.lincoln.form.md` - Filled political
     form
...
Since comprehensive automated testing covers the core functionality, manual validation
is minimal:
