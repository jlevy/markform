...
duplicate example metadata from `exampleRegistry.ts` by loading `title` and
`description` dynamically from YAML frontmatter.
...
2. **Frontmatter metadata loading** -
   `each example has title and description in frontmatter`
   - Verifies `loadExampleMetadata()` successfully extracts title and description from
     each example’s YAML frontmatter
...
The `pnpm precommit` script runs:
