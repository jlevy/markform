...
This PR upgrades zod from v3.25.76 to v4.2.1 and vitest from v3.2.4 to v4.0.16, along
with several other dependency updates.
...
**Low Risk**: All changes are dependency upgrades with well-documented migration paths.
All 682 tests pass locally with no test code changes required (except one test data fix
for stricter zod validation).
