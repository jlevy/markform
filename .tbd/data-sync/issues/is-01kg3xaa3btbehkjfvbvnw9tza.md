---
close_reason: null
closed_at: 2025-12-30T00:18:01.496Z
created_at: 2025-12-29T23:59:27.066Z
deferred_until: null
dependencies: []
due_date: null
extensions:
  beads:
    imported_at: 2026-01-29T03:38:58.431Z
    original_id: markform-470
id: is-01kg3xaa3btbehkjfvbvnw9tza
kind: task
labels: []
parent_id: null
priority: 2
status: closed
title: Add run_mode to bundled example forms
type: is
updated_at: 2025-12-30T00:18:01.496Z
version: 1
---
Add `run_mode` to all bundled example forms' frontmatter.

**Mapping:**
| Example | run_mode |
|---------|----------|
| simple.form.md | fill |
| movie-research-*.form.md | research |
| earnings-analysis.form.md | research |
| startup-deep-research.form.md | research |
| celebrity-deep-research.form.md | research |

**Files:**
- packages/markform/examples/simple/simple.form.md
- packages/markform/examples/movie-research/*.form.md
- packages/markform/examples/earnings-analysis/earnings-analysis.form.md
- packages/markform/examples/startup-deep-research/startup-deep-research.form.md
- packages/markform/examples/celebrity-deep-research/celebrity-deep-research.form.md

**Ref:** markform-462
